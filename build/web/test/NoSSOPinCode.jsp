...
<%
    ...
    String MDN1 = request.getParameter("MDN1");
    String MDN2 = request.getParameter("MDN2");
   ...
   // String tmprequestBody = "MDN1=" + MDN1 + "&MDN2=" + MDN2 + "&CPLIBM=" + CPLIBM + "&SALESID=" + SALESID + "&STOREID=" + STOREID + "&STORENAME=" + STORENAME + "&APISRCID=" + APISRCID;


    String tmprequestBody = "<ServicePinCodeDeposit><MDN1>"+MDN1+"</MDN1>"
            + "<MDN2>"+MDN2+"</MDN2>"
            + "<CPLIBM>"+CPLIBM+"</CPLIBM>"
            + "<SALESID>"+SALESID+"</SALESID>"
            + "<STOREID>"+STOREID+"</STOREID>"            
            + "<APISRCID>"+APISRCID+"</APISRCID>" 
            + "<STORENAME>"+STORENAME+"</STORENAME></ServicePinCodeDeposit>";

    String requestBody = util.encrypt(deskey, tmprequestBody);

    String XML_DOWNSTREM_URL = "http://epaytde.aptg.com.tw/EPAY/nosso/OnlineNoSSO_PinCodeDeposit.zhtml?CPID=" + CPID;
     ...
     * doc = (Document)docBuilder.parse((new ByteArrayInputStream(
     * responseBody.getBytes("utf-8") ))); Element element =
     * doc.getDocumentElement(); out.println(element.getNodeName()); NodeList
     * personNodes = element.getChildNodes(); for (int i=0;
     * i<personNodes.getLength(); i++){ Node emp = personNodes.item(i); if
     * (isTextNode(emp)) continue; out.println( emp.getNodeName()+"</br>" );
     * NodeList NameDOBCity = emp.getChildNodes(); for (int j=0;
     * j<NameDOBCity.getLength(); j++ ){ Node node = NameDOBCity.item(j); if (
     * isTextNode(node)) continue; out.println(
     * emp.getNodeName()+util.decrypt(deskey,
     * node.getFirstChild().getNodeValue())+"</br>" ); } }
     */
   // System.out.println("@@@@: requestBody = " + requestBody);
   ...
   // out.println("@@@@: responseBody = <textarea rows=30 cols=150>" + decode_resp + "</textarea>");
...
<html>
    ...
    <body>
        <form action="<%=XML_DOWNSTREM_URL%>" method="POST"> 
           <input type="hidden" id="DATA" name="DATA" value="<%=requestBody%>" /> 
        </form>
