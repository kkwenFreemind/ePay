...
public class AESUtil {
    ...
    public static boolean isValidKeySize(int keySize) {
        if (keySize == AESUtil.KEY_SIZE_128
                || keySize == AESUtil.KEY_SIZE_192
                || keySize == AESUtil.KEY_SIZE_256
                || keySize == AESUtil.KEY_SIZE_512
                || keySize == AESUtil.KEY_SIZE_1024
                || keySize == AESUtil.KEY_SIZE_2048
                || keySize == AESUtil.KEY_SIZE_4096) {
            return true;
        } else {
            return false;
        }
    ...
    public AESUtil(int iBlockLength) {
        switch (iBlockLength) {
            case KEY_SIZE_128:
                Nk = 4;
                Nb = 4;
    ...
    /**
     * encrypt the input message (bytesMessage) the procedure is change the message in the packet format listed below, and then use aes to encrypt |message size|version of richard AES|message body| 0------------7---------------------11------------- we need to record message size becord aes is a block cipher algorithm, if you don't remember the original message size, you might get lot of 0x00 at your decrypted message.
     *
    ...
    /**
     * decrypt the input message (bytesMessage) the procedure is use aes to decrypt and then get the message form the packet. the packet format listed below: |message size|version of richard AES|message body| 0------------7---------------------11------------- we need to record message size becord aes is a block cipher algorithm, if you don't remember the original message size, you might get lot of 0x00 at your decrypted message.
     *
    ...
    public static String Encrypt(String str) throws Exception {
        AESUtil alg = new AESUtil(AESUtil.KEY_SIZE_128); // up to 4096

        byte[] bytesKey = alg.createKey();
        int[] wordsKeyExpansion = alg.createKeyExpansion(bytesKey);

        //  byte[] key = base64Decode(inkey);
        //   int[] wordsKeyExpansion = alg.createKeyExpansion(key);
        byte[] bytesMessage = str.getBytes();
        byte[] bytesEncrypted = alg.cipher(bytesMessage, wordsKeyExpansion);
        //  System.out.println("@@@@@@@base64Encode(bytesEncrypted):" + base64Encode(bytesEncrypted).length());
        return (base64Encode(bytesEncrypted) + base64Encode(bytesKey));
    }
...
//        int[] wordsKeyExpansion = alg.createKeyExpansion(bytesKey);
        ...
        byte[] key = base64Decode(str.substring(44));
        int[] wordsKeyExpansion = alg.createKeyExpansion(key);
        byte[] bytesDecrypted = alg.invCipher(base64Decode(str.substring(0, 44)), wordsKeyExpansion);

        return new String(bytesDecrypted, "utf-8");
    }

    public static String Decrypt_64(String str) throws Exception {
        AESUtil alg = new AESUtil(AESUtil.KEY_SIZE_128); // up to 4096
//        byte[] bytesKey = alg.createKey();
//        int[] wordsKeyExpansion = alg.createKeyExpansion(bytesKey);
        //  String strMessage =str;
        ...
        byte[] key = base64Decode(str.substring(64));
        int[] wordsKeyExpansion = alg.createKeyExpansion(key);
        byte[] bytesDecrypted = alg.invCipher(base64Decode(str.substring(0, 64)), wordsKeyExpansion);

        return new String(bytesDecrypted, "utf-8");
    }

    public static String Decrypt_24(String str) throws Exception {
        AESUtil alg = new AESUtil(AESUtil.KEY_SIZE_128); // up to 4096
//        byte[] bytesKey = alg.createKey();
//        int[] wordsKeyExpansion = alg.createKeyExpansion(bytesKey);
        //  String strMessage =str;
        ...
        byte[] key = base64Decode(str.substring(24));
        int[] wordsKeyExpansion = alg.createKeyExpansion(key);
        byte[] bytesDecrypted = alg.invCipher(base64Decode(str.substring(0, 24)), wordsKeyExpansion);
